...
Status: implemented on `feat/mvp-implementation`.

This document is now historical. It captures the initial plan and many of the
right boundaries, but the implementation deliberately simplified several parts
after real review and usage testing. The current code is the source of truth for
the MVP architecture.

Current MVP shape:

- `packages/harness` is the core package: event persistence, provider
abstraction, prompt building, tool runtime, and the invocation loop.
- `apps/cli` is the first app: command parsing, live terminal rendering, and
concrete tools (`read_file`, `list_dir`, `bash`).
- `lh` is the launcher. `lh` and `lh cli` start the interactive CLI; `lh "<prompt>"` runs one prompt; `--session <id>` resumes a session.
- Session state is append-only JSONL at `.leharness/sessions/{id}/events.jsonl`.
- Events are intentionally loose: an event has an envelope (`v`, `id`, `ts`,
`type`) plus payload fields. Consumers interpret only the event types they
care about.
- There is no reducer/session/transcript module. `buildPrompt(events, tools, options)` owns event-to-provider-message projection internally.
- `runInvocation` keeps only `events` as mutable session state, records events
explicitly in the harness loop, and returns the persisted event list.
- Tool execution does not emit events internally. `executeToolCalls` returns
structured results, and the harness loop records `tool.completed` or
`tool.failed`.
- Streaming is implemented through `ProviderRequest.onText` and rendered by the
CLI.
- Verification is smoke-script based (`pnpm smoke`) plus typecheck, lint, and
Knip in pull-request CI. There are no colocated Vitest suites in the MVP.

Major intentional deviations from the original plan:

- The initial CLI app is `apps/cli`.
- `cli` is the only interactive app subcommand.
- Reducer/session/transcript projection was removed to keep event flow simpler.
- `model.requested` and `tool.started` were dropped as audit-only events.
- The max-step cap and graceful in-turn interrupt were deferred. Ctrl-C exits
the process; session resume picks up from the persisted log.
- TUI, compaction, MCP, skills, approvals, background tasks, subagents,
artifacts, and richer app surfaces remain future work.

The rest of this file should be read as the planning path that led to the MVP,
not as an exact description of the final implementation.
...
| Repo layout             | `packages/harness` (kernel only) + `apps/cli` (concrete tools + CLI wiring)                                                                                                                                               |
...
| Tool ownership          | Harness ships the tool *runtime* (interface, executor); concrete tools (`read_file`, `list_dir`, `bash`) live in the consumer (`apps/cli`)                                                                                |
...
│       └── package.json
├── apps/
│   └── cli/                     # Basic stdin/argv parsing only, no UI/UX
│       ├── src/
│       │   ├── tools/
│       │   │   ├── read_file.ts
│       │   │   ├── list_dir.ts
...
│       │   ├── cli.ts           # arg parsing, interactive CLI, wires provider + tools into runInvocation
...
1. Implement the three builtin tools in `apps/cli/src/tools/`:
...
3. `apps/cli/src/cli.ts` parses args:
  - `lh "<prompt>"` — one-shot, runs `runInvocation` once, prints
   final assistant message.
  - `lh cli` — interactive CLI: `while (input = await readLine()) await runInvocation(sessionId, input, deps)`.
...
5. `apps/cli/src/render.ts` — plain-text output, one line per event:
...
6. Subscribe to events via a simple file-watcher or by reading the log after
  each loop iteration. (TUI does this properly in Phase 8; CLI
   stays dumb on purpose.)
7. Per-tool unit tests live in `apps/cli` (fixture filesystems for
...
5. Zero coupling to the loop. `apps/cli` runs the loop in one terminal, TUI
...
the CLI loop appends events the TUI updates live. The TUI can be killed
