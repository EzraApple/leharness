...
console.log(`smoke-basic: LEHARNESS_HOME = ${tmp}`)
...
console.log(`smoke-basic: jsonl path = ${jsonlPath}`)
const jsonlStat = await fs.stat(jsonlPath)
console.log(`smoke-basic: jsonl size = ${jsonlStat.size} bytes`)

const raw = await fs.readFile(jsonlPath, "utf8")
const lines = raw.split("\n").filter((l) => l.length > 0)
console.log(`smoke-basic: ${lines.length} jsonl lines`)
...
console.log(`smoke-basic: loadEvents returned ${events.length} events`)

const directoryExists = await fs
  .stat(path.join(tmp, "sessions", sessionId))
  .then(() => true)
  .catch(() => false)
console.log(`smoke-basic: session directory exists = ${directoryExists}`)
